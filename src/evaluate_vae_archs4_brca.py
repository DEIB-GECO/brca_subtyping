...
for drop in d_rates:
	for drop2 in d_rates2:
		...
		for train_index, test_index in skf.split(X_brca_train, y_brca_train):
			...
			filename="../results/10-fold/RNA/VAE/{}_hidden_{}_emb/history/archs4_brca_classifier_dropout_{}_in_{}_hidden_rec_loss_{}_history_{}_classifier_frozen_{}_cv_other_metrics.csv".format(hidden_dim, latent_dim, dropout_input, dropout_hidden, reconstruction_loss, i, vae.freeze_weights)
			history_df.to_csv(filename, sep=',')
		...
		output_filename="../results/10-fold/RNA/VAE/{}_hidden_{}_emb/archs4_brca_classifier_dropout_{}_in_{}_hidden_rec_loss_{}_cv_other_metrics.csv".format(hidden_dim, latent_dim, dropout_input, dropout_hidden, reconstruction_loss)

		conf_filename="../results/10-fold/RNA/VAE/{}_hidden_{}_emb/confusion_matrix/archs4_brca_classifier_dropout_{}_in_{}_hidden_rec_loss_{}_cv_confusion_matrix_other_metrics.csv".format(hidden_dim, latent_dim, dropout_input, dropout_hidden, reconstruction_loss)
...
# Prepare data to train Variational Autoencoder (merge dataframes and normalize)
X_autoencoder = pd.concat([X_brca_train, X_archs4_brca], sort=True)
scaler = MinMaxScaler()
...
score = vae.classifier.evaluate(X_test, y_labels_test)
conf_matrix = pd.DataFrame(confusion_matrix(y_labels_test.values.argmax(axis=1), vae.classifier.predict(X_test).argmax(axis=1)))

conf_matrix.to_csv("../results/VAE/{}_hidden_{}_emb/archs4_brca_classifier_dropout_{}_in_{}_hidden_rec_loss_{}_classifier_frozen_{}_confusion_matrix.csv".format(hidden_dim, latent_dim, dropout_input, dropout_hidden, reconstruction_loss, freeze_weights)
...
output_filename="../results/VAE/{}_hidden_{}_emb/archs4_brca_classifier_dropout_{}_in_{}_hidden_rec_loss_{}_classifier_frozen_{}_test_final.csv".format(hidden_dim, latent_dim, dropout_input, dropout_hidden, reconstruction_loss, freeze_weights)
