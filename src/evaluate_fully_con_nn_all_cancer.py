...
for drop1 in d_rates1:
	for drop2 in d_rates2:
		...
		for train_index, test_index in skf.split(X_all_train, y_all_train):
			print('Fold {} of {}'.format(i, skf.n_splits))

			X_train, X_val = X_all_train.iloc[train_index], X_all_train.iloc[test_index]
			y_train, y_val = y_all_train.iloc[train_index], y_all_train.iloc[test_index]
		...
		output_filename="../results/fully_con/all_cancer/{}_hidden_{}_emb_tcga_all_cancer_classifier_dropout_{}_in_{}_hidden_cv.csv".format(hidden_dim_1, hidden_dim_2, dropout_input, dropout_hidden)
